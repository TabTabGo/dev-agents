...
## Variables
**org**: Your Azure DevOps organization name take it from .env AZURE_DEVOPS_ORG_URL it not from root folder settings.json
**project**: Your Azure DevOps project name take it from {project folder}/settings.json variable name "devOpsProjectName"
...
| `references/boards-and-iterations.md` | Work items, sprints, area paths, Iterations | Work items (WIQL/create/update/relations), Area paths, Iterations, Team iterations |
| `references/team-workitems.md` | Team-oriented WIQL recipes, iteration naming, story/task creation | Work items by user/status/iteration (grouped or filtered), iteration status, feature/release iteration conventions, User Story + child Tasks pattern |
